...
public class Peaklass {
	...
	public static void kaartideJarjekord(List<Kaart> kaardid, Pakk pakk, int mangijaNr, Scanner sisend) { //pakk - sinna lisatakse uued kaardid, mangijaNr - kumma mängija kaardid
		...
		for(int i = 0; i < kaardid.size(); i++) {
			...
			int indeksNumbrina = Integer.parseInt(indeksSonena);
			if(mangijaNr == 1){
...
//			System.out.println(i);
	...
	public static void nimi(int kaartideVordlus, Kaart kaart1, Kaart kaart2, Pakk pakk, Scanner sisend, List<Kaart> voidetavadKaardid, List<Kaart> mangija1Kaardid, List<Kaart> mangija2Kaardid) {
		if(kaartideVordlus == 1){
			//kaartideJarjekord(Arrays.asList(kaart1, kaart2), pakk, 1, sisend);
			kaartideJarjekord(voidetavadKaardid, pakk, 1, sisend);
		}
		if(kaartideVordlus == 2){
			//kaartideJarjekord(Arrays.asList(kaart1, kaart2), pakk, 2, sisend);
			kaartideJarjekord(voidetavadKaardid, pakk, 2, sisend);
		}
		if(kaartideVordlus == 0){
			...
			nimi(vordleKaarte(uusKaart1, uusKaart2), uusKaart1, uusKaart2, pakk, sisend, kokkuKaardid, kaardid1, kaardid2);
		}
	}
	...
	public static void mangi() {
		...
		while(true){
			...
			nimi(kaartideVordlus, kaart1, kaart2, pakk, sisend, Arrays.asList(kaart1, kaart2), Arrays.asList(kaart1), Arrays.asList(kaart2));
